...
------- Ако искате да ползвате и команда, изтрийте тези скоби отдолу --[[ --]]
--[[RegisterCommand("rc", function()
	RCCar.Start()
end)--]]

RegisterNetEvent('no-limitrp:rcCar')
AddEventHandler('no-limitrp:rcCar', function()
    RCCar.Start()
end)
...
RCCar.HandleKeys = function(distanceCheck)
	...
	if distanceCheck <= 1.5 then
		if IsControlJustPressed(0, 38) then
			RCCar.Attach("pick")
			TriggerServerEvent("no-limitrp:get")
		end
	end
